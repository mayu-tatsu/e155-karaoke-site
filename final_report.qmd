...
Evidence of our project's functionality can be viewed in the following [Figure 9](#fig-video), with the tester, [Alan Kappler](https://www.linkedin.com/in/alan-kappler-5855aa268/), attempting to sing *Golden*:
...
Additionally, [Figure 10](#fig-final-build) provides a close-up photo of our machine:
...
First and foremost, we have the [Figure 11](#fig-cic-tb) waveform outputs of our CIC filter.
...
Next, we have our half-band filter in [Figure 12](#fig-hb-tb).
...
Again, with our [Figure 13](#fig-fir-tb) FIR testbench output, you can see that the validity signal gets decimated by 2 down to 16 kHz and that the output signal gets choppier as a result.
...
Finally, with the [Figure 14](#fig-spi-tb) image, we can see evidence of our SPI transaction's success.
...
Last but not least, we used a Logic Analyzer in [Figure 15](#fig-lcd-logic-analyzer) to aid us in proving that the LCD works as expected.
...
Evidently, we can see that after `E` pulses for the first time (which is actually signified on `D8` in the above image), the traces match exactly what we expected.
